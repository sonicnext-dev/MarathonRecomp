        ...
        if (enableAura)
        {
            ...
            if (Config::EnableColoredAuraForGems)
            {
                ...
                // Default rainbow
                    ...
                    ProcessBothParts([&](auto& part) {
                        if (part.ESPState != 2) {
                            ProcessESPPart(part, RainbowPalette, part.ESPState == 0 ? 2.0f : 6.0f);
                        }
                        });
                }
            }
...
bool ModelSonic_OnContextFlagGPFix(PPCRegister& model, PPCRegister& gem_index)
{
    ...
    std::map<std::string, std::vector< Sonicteam::SoX::Graphics::FrameObserver*>> observerMap;
    pModel->m_spRootFrame->ForEach([&](Sonicteam::SoX::Graphics::Frame* frame)
        {
            for (auto& it : frame->m_llFrameObserver)
            {
                ...
                {
                    printf("class Sonicteam::Player::Effect::ParticleJoint Saved From LOL\n");
                    printf("frame : %p\n", frame);
                    ...
                    observerMap[frame->GetName() == 0 ? "" : frame->GetName()].push_back(it.m_pThis.get());
                }
            }
        });
