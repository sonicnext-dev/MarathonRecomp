...
namespace Sonicteam::SoX::Graphics
{
    ...
    // Fixed hierarchy methods with better names
    inline void Frame::AttachChild(Frame* pChild)
    {
        if (!pChild) return;
        ...
        if (m_pFirstChild) {
            ...
            // Insert between lastChild and firstChild
            lastChild->m_pNextSibling = pChild;
            pChild->m_pPrevSibling = lastChild;
            pChild->m_pNextSibling = m_pFirstChild;
            m_pFirstChild->m_pPrevSibling = pChild;
        }
        else {
            // First child - self-referential circular list
            m_pFirstChild = pChild;
            pChild->m_pPrevSibling = pChild;
            pChild->m_pNextSibling = pChild;
        }

        // Set parent and update ownership
        pChild->m_pParent = this;
        pChild->UpdateOwnerRecursive(m_pOwner);

        // Handle synchronization
        if (pChild->m_pSynchronizer) {
            pChild->m_Flag = pChild->m_Flag.get() | FrameFlags::FLAG_SYNC_ENABLED;
            pChild->m_pSynchronizer->RemoveFromPriorityQueue(pChild);
        }

        // Link into owner's frame list if needed
        if (pChild->m_pOwner && !pChild->m_pOwner->m_lnFrame2.m_pNext) {
            auto frameNode = pChild->m_pOwner->m_plnFrame;
            if (frameNode) {
                auto& frame2 = pChild->m_pOwner->m_lnFrame2;
                frame2.m_pPrev = frameNode;
                frame2.m_pNext = frameNode->m_pNext;
                frameNode->m_pNext->m_pPrev = &frame2;
                frameNode->m_pNext = &frame2;
        ...
        // Update flags and reference count
        if (pChild->m_pOwner) {
            pChild->m_pOwner->m_Flag = pChild->m_pOwner->m_Flag.get() | FrameFlags::FLAG_ACTIVE;
        }

        pChild->m_Flag = pChild->m_Flag.get() | FrameFlags::FLAG_VISIBLE;
        pChild->m_ReferenceCount = pChild->m_ReferenceCount.get() + 1;
    }
    ...
    inline void Frame::SwapWith(Frame* pOtherFrame)
    {
        if (!pOtherFrame) return;

        Frame* myParent = m_pParent;
        Frame* otherParent = pOtherFrame->m_pParent;

        // Remove both from current parents
        if (myParent) RemoveFromParent();
        if (otherParent) pOtherFrame->RemoveFromParent();

        // Swap parents
        if (myParent) myParent->AttachChild(pOtherFrame);
        if (otherParent) otherParent->AttachChild(this);
    }
    ...
    inline bool Frame::HasDirectChild(Frame* pFrame)
    {
        ...
        // Check only direct children (no recursion)
        for (Frame* child = m_pFirstChild.get(); child; child = child->m_pNextSibling.get())
        {
            if (child == pFrame)
                return true;
    ...
    inline void Frame::RemoveNodeByName(const char* name)
    {
        ...
        if (node && node != this) // Don't remove self
        {
            node->RemoveFromParent();
        }
    }

    // Circular list management helpers
    inline void Frame::InsertChildBefore(Frame* pNewChild, Frame* pReferenceChild)
    {
        if (!pNewChild || !pReferenceChild || pReferenceChild->m_pParent != this) return;

        pNewChild->m_pPrevSibling = pReferenceChild->m_pPrevSibling;
        pNewChild->m_pNextSibling = pReferenceChild;
        pReferenceChild->m_pPrevSibling->m_pNextSibling = pNewChild;
        pReferenceChild->m_pPrevSibling = pNewChild;

        if (m_pFirstChild == pReferenceChild) {
            m_pFirstChild = pNewChild;
        }

        // Set parent and update other properties
        pNewChild->m_pParent = this;
        pNewChild->UpdateOwnerRecursive(m_pOwner);
        pNewChild->m_ReferenceCount = pNewChild->m_ReferenceCount.get() + 1;
    }

    inline void Frame::InsertChildAfter(Frame* pNewChild, Frame* pReferenceChild)
    {
        if (!pNewChild || !pReferenceChild || pReferenceChild->m_pParent != this) return;

        pNewChild->m_pPrevSibling = pReferenceChild;
        pNewChild->m_pNextSibling = pReferenceChild->m_pNextSibling;
        pReferenceChild->m_pNextSibling->m_pPrevSibling = pNewChild;
        pReferenceChild->m_pNextSibling = pNewChild;
    ...
    inline Frame* Frame::GetLastChild() const
    {
        return m_pFirstChild ? m_pFirstChild->m_pPrevSibling : nullptr;
    }

    inline size_t Frame::GetChildCount() const
    {
        ...
        do {
            count++;
            child = child->m_pNextSibling;
        } while (child != m_pFirstChild);

        return count;
    }

    inline bool Frame::IsCircularListValid() const
    {
        if (!m_pFirstChild) return true;

        Frame* child = m_pFirstChild;
        Frame* last = m_pFirstChild->m_pPrevSibling;

        // Check if last child points back to first
        return last->m_pNextSibling == m_pFirstChild;
    }

    inline void Frame::SafeRemove()
    {
        if (m_pSynchronizer && (m_Flag.get() & FrameFlags::FLAG_SYNC_ENABLED)) {
            // Let synchronizer handle the removal
            m_pSynchronizer->RemoveFromPriorityQueue(this);
        }
    ...
    inline Frame* Frame::FindNodeCaseInsensitive(const char* name)
    {
        if (auto currentName = GetName())
        {
        ...
        for (Frame* child = m_pFirstChild.get(); child; child = child->m_pNextSibling.get())
        {
            if (Frame* found = child->FindNodeCaseInsensitive(name))
                return found;
        }
    ...
    inline Frame* Frame::FindNodePartial(const char* partialName)
    {
        if (auto currentName = GetName())
        {
            if (strstr(currentName, partialName) != nullptr)
                return this;
        }

        for (Frame* child = m_pFirstChild.get(); child; child = child->m_pNextSibling.get())
        {
            if (Frame* found = child->FindNodePartial(partialName))
                return found;
        }
